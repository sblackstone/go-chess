...
func genPerfMakeUnmake(b *boardstate.BoardState, depth int, posMap map[uint64]int, moves []*boardstate.Move) int {
	...
	for _, move := range moves {
		...
		if !IsInCheck(b, b.EnemyColor()) {
			if depth == 1 {
				...
				result += genPerfMakeUnmake(b, depth-1, posMap, moves[len(moves):])
			}
		}
		b.UnplayTurn()
	}
...
func BenchmarkPerfMakeUnmake(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		backingSlice := make([]*boardstate.Move, 0, 1000)
		v := genPerfMakeUnmake(board, 6, posMap, backingSlice)
		b.ReportMetric(float64(v), "positions")
